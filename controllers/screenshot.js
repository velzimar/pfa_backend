    ...
    const storage = multer.diskStorage({
        destination: function(req, file, cb) {
            cb(null, 'C:/Users/ALVES/Documents/GitHub/pfa_backend/uploads');
        },
    ...
    exports.getAllScreenshot =function(req, res){
        Screenshot.find()
        .select("title description _id requRes_id screenshot")
        .exec()
        .then(docs => {
        const response = {
            count: docs.length,
...
exports.createScreenshot=function(req, res) {
    let upload = multer({ storage: storage, fileFilter: fileFilter, limits: {fileSize: 1024 * 1024 * 5} }).single('screenshot');
...
/*
    ...
    exports.deleteScreenshot =function destroy(req, res) {
        const { id } = req.params;
        var fs = require('fs');
        Screenshot.findOneAndRemove({ id },{useFindAndModify: false})
         .then(screenshot => {
          res.json(screenshot);
          console.log(screenshot.screenshot);
